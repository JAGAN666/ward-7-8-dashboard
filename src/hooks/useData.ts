...
// =====================================================
// HISTORICAL TIME SERIES DATA HOOKS
// =====================================================
...
// Type for metric time series
export interface MetricTimeSeries {
  name: string;
  data: TimeSeriesDataPoint[];
}

// Type for full time series response
export interface TimeSeriesResponse {
  [metricCode: string]: MetricTimeSeries;
}

// Hook for historical ACS time series data
export function useHistoricalData() {
  return useQuery({
    queryKey: ['historicalTimeSeries'],
    queryFn: () => fetchJSON<TimeSeriesResponse>('historical/metrics_timeseries.json'),
    staleTime: Infinity,
  });
}

// Helper to get BOTG data for a specific metric
export function getMetricBOTG(
  timeSeriesData: TimeSeriesResponse | null,
  metricCode: string,
  ward: 7 | 8
): { year: number; value: number | null; dc: number | null }[] {
  if (!timeSeriesData || !timeSeriesData[metricCode]) {
    return [];
  }

  const metric = timeSeriesData[metricCode];
  return metric.data
    .filter(d => d.year && (d.ward7 !== undefined || d.ward8 !== undefined))
    .map(d => ({
      year: d.year,
      value: ward === 7 ? (d.ward7 ?? null) : (d.ward8 ?? null),
      dc: d.dc ?? null,
    }))
...
// Helper to format BOTG data for LineChart component
export function formatBOTGForChart(
  timeSeriesData: TimeSeriesResponse | null,
  metricCode: string,
  ward: 7 | 8
): { year: number; ward7: number; ward8: number; dcAverage?: number }[] {
  if (!timeSeriesData || !timeSeriesData[metricCode]) {
    return [];
  }
  ...
  const metric = timeSeriesData[metricCode];
  return metric.data
    .filter(d => d.year && (ward === 7 ? d.ward7 !== undefined : d.ward8 !== undefined))
    .map(d => ({
      year: d.year,
      ward7: ward === 7 ? (d.ward7 ?? 0) : 0,
      ward8: ward === 8 ? (d.ward8 ?? 0) : 0,
      dcAverage: d.dc ?? undefined,
    }))
    .filter(d => (ward === 7 ? d.ward7 > 0 : d.ward8 > 0))
    .sort((a, b) => a.year - b.year);
}

// Helper to calculate derived BOTG data (like rent burdened rate over time)
export function getDerivedBOTG(
  timeSeriesData: TimeSeriesResponse | null,
  numeratorCode: string,
  denominatorCode: string,
  ward: 7 | 8
): { year: number; ward7: number; ward8: number; dcAverage?: number }[] {
  if (!timeSeriesData || !timeSeriesData[numeratorCode] || !timeSeriesData[denominatorCode]) {
    return [];
  }

  const numerator = timeSeriesData[numeratorCode].data;
  const denominator = timeSeriesData[denominatorCode].data;

  const result: { year: number; ward7: number; ward8: number; dcAverage?: number }[] = [];

  for (const numData of numerator) {
    const denData = denominator.find(d => d.year === numData.year);
    if (!denData) continue;

    const wardKey = ward === 7 ? 'ward7' : 'ward8';
    const numValue = numData[wardKey];
    const denValue = denData[wardKey];
    const dcNum = numData.dc;
    const dcDen = denData.dc;

    if (numValue != null && denValue != null && denValue > 0) {
      const rate = (numValue / denValue) * 100;
      const dcRate = dcNum != null && dcDen != null && dcDen > 0
        ? (dcNum / dcDen) * 100
        : undefined;
...
// Combined hook for Ward page with historical data
export function useWardPageData(ward: 7 | 8) {
  ...
  const { data: historicalData, isLoading: loadingHistorical } = useHistoricalData();

  const isLoading = loadingWard || loadingCrime || loadingFood || loadingHistorical;

  // Calculate derived metrics from historical data
  const getLatestValue = (metricCode: string): number | null => {
    if (!historicalData || !historicalData[metricCode]) return null;
    const data = historicalData[metricCode].data;
    const latestWithValue = data.find(d => (ward === 7 ? d.ward7 : d.ward8) !== undefined);
    return latestWithValue ? (ward === 7 ? latestWithValue.ward7 : latestWithValue.ward8) ?? null : null;
  };

  const getDCValue = (metricCode: string): number | null => {
    if (!historicalData || !historicalData[metricCode]) return null;
    const data = historicalData[metricCode].data;
    const latestWithValue = data.find(d => d.dc !== undefined);
    return latestWithValue?.dc ?? null;
  };

  // Calculate rent burdened rate
  const rentBurdened = getLatestValue('DP04_0142E');
  const occupiedPayingRent = getLatestValue('DP04_0136E');
  const rentBurdenedRate = rentBurdened && occupiedPayingRent ? (rentBurdened / occupiedPayingRent) * 100 : null;

  const dcRentBurdened = getDCValue('DP04_0142E');
  const dcOccupiedPayingRent = getDCValue('DP04_0136E');
  const dcRentBurdenedRate = dcRentBurdened && dcOccupiedPayingRent ? (dcRentBurdened / dcOccupiedPayingRent) * 100 : null;

  // Calculate occupancy rate
  const occupiedUnits = getLatestValue('DP04_0002E');
  const totalUnits = getLatestValue('DP04_0001E');
  const occupancyRate = occupiedUnits && totalUnits ? (occupiedUnits / totalUnits) * 100 : null;

  const dcOccupiedUnits = getDCValue('DP04_0002E');
  const dcTotalUnits = getDCValue('DP04_0001E');
  const dcOccupancyRate = dcOccupiedUnits && dcTotalUnits ? (dcOccupiedUnits / dcTotalUnits) * 100 : null;

  // Calculate homeownership rate
  const ownerOccupied = getLatestValue('DP04_0046E');
  const occupiedTenure = getLatestValue('DP04_0045E');
  const homeownershipRate = ownerOccupied && occupiedTenure ? (ownerOccupied / occupiedTenure) * 100 : null;

  const dcOwnerOccupied = getDCValue('DP04_0046E');
  const dcOccupiedTenure = getDCValue('DP04_0045E');
  const dcHomeownershipRate = dcOwnerOccupied && dcOccupiedTenure ? (dcOwnerOccupied / dcOccupiedTenure) * 100 : null;

  // Calculate early childhood education rate
  const nursery = getLatestValue('DP02_0054E');
  const kindergarten = getLatestValue('DP02_0055E');
  const under5 = getLatestValue('DP05_0005E');
  const earlyChildhoodRate = nursery && kindergarten && under5 ? ((nursery + kindergarten) / under5) * 100 : null;

  const dcNursery = getDCValue('DP02_0054E');
  const dcKindergarten = getDCValue('DP02_0055E');
  const dcUnder5 = getDCValue('DP05_0005E');
  const dcEarlyChildhoodRate = dcNursery && dcKindergarten && dcUnder5 ? ((dcNursery + dcKindergarten) / dcUnder5) * 100 : null;
  ...
  return {
    ...
    derivedMetrics: {
      ...
      dcHomeownershipRate,
      earlyChildhoodRate,
      dcEarlyChildhoodRate,
      population: getLatestValue('DP05_0001E'),
      dcPopulation: getDCValue('DP05_0001E'),
    },
    // Helper to get BOTG chart data
    getBOTGData: (metricCode: string) => formatBOTGForChart(historicalData ?? null, metricCode, ward),
    // Helper to get derived BOTG data (for calculated rates like rent burdened)
    getDerivedBOTGData: (numeratorCode: string, denominatorCode: string) =>
      getDerivedBOTG(historicalData ?? null, numeratorCode, denominatorCode, ward),
  };
}
