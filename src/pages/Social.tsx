...
import { useSocialAnalysis } from '../hooks/useData';
import { MetricCategorySection } from '../components/common/MetricCard';

export function Social() {
  const { metrics, metricsByCategory, isLoading } = useSocialAnalysis();
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading social data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-purple-900 via-purple-800 to-slate-800 text-white py-12">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-xl text-purple-200 max-w-3xl">
            Social and educational metrics including household composition, marital status,
            educational attainment, disability status, veteran status, and ancestry.
          </p>
          <div className="mt-6 flex items-center gap-4 text-sm">
            ...
            <span className="bg-purple-700/50 px-3 py-1 rounded-full">
              Source: ACS 5-Year Estimates (DP02)
            </span>
          </div>
        </div>
      </section>
